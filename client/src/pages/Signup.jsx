...
const Signup = () => {
  ...
  return (
    <div className="relative flex min-h-[85vh] items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {['Student', 'Admin', 'Visitor'].map((roleOption) => (
                  <button
                    key={roleOption}
                    type="button"
                    onClick={() => handleRoleSelect(roleOption)}
                    disabled={isLoading}
                    className={`rounded-lg py-2 text-xs font-semibold tracking-wide border transition-all duration-300 cursor-pointer hover:scale-[1.02] active:scale-[0.98] ${
                      formData.role === roleOption
                        ? 'bg-violet-650/10 dark:bg-violet-600/20 text-violet-600 dark:text-violet-400 border-violet-500/50 shadow-md shadow-violet-500/5'
                        : 'bg-slate-900/10 dark:bg-slate-900/40 text-muted border-theme hover:border-slate-400 hover:text-heading'
                    }`}
                  >
                    {roleOption}
                  </button>
